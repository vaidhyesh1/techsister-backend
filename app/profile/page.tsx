...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { organisation, phone, industry, role, proficiency, mentor } = response.data.details[0];
        // form.reset({
        //   organisation,
        //   phone,
        //   industry,
        ...
        form.setValue('mentor', mentor);

        // Set the selected values for select fields
        form.setValue('industry', industry);
        form.setValue('role', role);
        form.setValue('proficiency', proficiency);
        setIsLoading(false);
      } catch (error) {
  ...
  useEffect(() => {
    // Replace with your session loading logic
    const sessionLoadingLogic = async () => {
      // Simulating session loading for 2 seconds
  ...
  const [mloading, setLoading] = React.useState(false);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (session) {
      if (values.mentor) {
        ...
        const res = await updateUser(data).then((res) => {
          setLoading(false);
          setTimeout(() => {
            router.push("/");
          }, 2000);
  ...
  return (
    ...
    <Form  {...form}>
      <div style={{marginTop: '10px'}} className="h-screen flex flex-col items-center space-y-3 max-w-xl mx-auto ">
        <h1 className="text-center text-2xl font-bold md:text-2xl">Update your profile</h1>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select
                  onValueChange={field.onChange}
                  value={form.getValues('proficiency')}
                >
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue placeholder="Select a role" />
